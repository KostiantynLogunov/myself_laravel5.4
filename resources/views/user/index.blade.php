...
{{--elements of animations--}}
    ...
    <div class="container text-center portfolio_title mt-5">
        ...
        <div class="row align-items-center">
            <div class="col-2">
                <img class="img-responsive" style="width: 150px;" src="{{'assets/img/users/'.Auth::user()->images}}" alt="user's foto">
            </div>
            <div class="col text-left " style="color: whitesmoke;">
                <ul>
                    <li>Your name: {{Auth::user()->name}}</li>
                    <li>Your Email: {{Auth::user()->email}}</li>
                    <li>Your registration date: {{Auth::user()->created_at}}</li>
...
{{--send sms to admin--}}
    {!! Form::open(['url'=>route('sendSms'),'class'=>'container pt-2','method'=>'POST','enctype'=>'multipart/form-data']) !!}

        <div class="form-group">
            {!! Form::label('text','Send message to Admin',['class'=>'col-xs-2 control-label'])!!}
            <div class="col-xs-8">
                {!! Form::textarea('text',old('text'),['id'=>'editor','class'=>'form-control','placeholder'=>'Enter messages for admin'])!!}
            </div>
        </div>
    ...
    <div class="container pt-3">
        @if(($messages->where('from_user_id',$from_id)->first()) || !empty($messages->where('to_user_id',$from_id)->first()) )
                ...
                @foreach($messages as $message)
                    @if($message['from_user_id']==$from_id)
                        <li class="list-group-item list-group-item-info mb-1 h-75">
                            <h5 style="color: darkblue">{{Auth::user()->name}} :</h5>
                            {{$message['text'] }}
                        </li>
                    @elseif($message['to_user_id']==$from_id)
